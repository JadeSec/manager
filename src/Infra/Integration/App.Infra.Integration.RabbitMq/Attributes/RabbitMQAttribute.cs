...
namespace App.Infra.Integration.RabbitMq.Attributes
{
    ...
    public class RabbitMQAttribute : Attribute
    {
        ...
        /// <returns></returns>
        public static RabbitMQAttribute Parse(Type type)
           => type.GetTypeInfo()
                  .GetCustomAttribute<RabbitMQAttribute>() ?? throw new ArgumentNullException($"Not exist attribute [EventBus(...)] in {type.GetType().Name}");
    }
}